...
CHUNK_FILE = "HydrateRidge_upper_2.02_MP_2min.csv" #"AlaskaFins_LA21_chunk.csv" #Name of saved call file
...
PLOTFLAG_STACK = True
CLIENT_CODE = 'IRIS'
network="OO" #Network name "OO" for OOI, "7D" for Cascadia, "XF" for PuertoRico, "XO" for alaska, "YO" for ENAM, "9A" for Hawaii, "ZZ" for puerto rico
station= "HYS11" #"LD41" # "B19" for PuertoRico station #Specific station, or '*' for all available stations "X06" for ENAM, "LOSW" for Hawaii, "XABV" for puerto rico
location='*'  # '*' for all available locations
channel= 'EHZ' #Choose channels,  you'll want 'BHZ,HHZ' for Cascadia 'ELZ' for hawaii
                      #Check http://ds.iris.edu/mda/OO/ for OOI station channels

#DET_PATH=cn.SCM_DETECTION.csv_path
DET_PATH="HydrateRidge_upper_2.02_MP_2min.csv" #"AlaskaFins_LA21.csv" #Final save file
...
STARTTIME = ("2020-02-02T17:09:00.000") #for testing on HR fins 3
ENDTIME = ("2020-02-02T21:00:00.000")

#STARTTIME = ("2019-12-29T16:00:00.000") #for testing on HR fins 2
#ENDTIME = ("2019-12-29T20:00:00.000")



HALF_HOUR = 1800  # in seconds
#CHUNK_LENGTH=HALF_HOUR/5 #secnods
CHUNK_LENGTH=60*2 #secnods
...
def main(STARTTIME, ENDTIME,
    ...
    if os.path.isfile(CHUNK_FILE) and is_restart:
        ...
        auto_df_full = [pd.read_csv('auto_HydrateRidge_upper_2.02_MP_2min.csv')]
        stack_df_full = [pd.read_csv('stack_HydrateRidge_upper_2.02_MP_2min.csv')]
    else:
        analyzers = []
        auto_df_full = []
        stack_df_full = []
    ...
    while utcend > utcstart_chunk:
        ...
        for idx in range(1, num_sta+1):
            ...
            if MP_SPCT_FLAG and len(analyzer_j.df) > 1: #if MP_FLAG is True
                ...
                for det in range(0,len(analyzer_j.df)):
                    mp_event=analyzer_j.mp_picker(timesnew, valuesnew, utcstart_chunk, dur=.01, prominence=0,distance=.1,rel_height=.1)
                ...
                for det in range(0,len(analyzer_j.df)):
                    mp_event=analyzer_j.mp_picker(dettimes_chunk,autocorr_chunk, utcstart_chunk, dur=.1, prominence=0,distance=.1,rel_height=.1)
                ...
                new_auto_df.to_csv('auto_HydrateRidge_upper_2.02_MP_2min.csv', index=False)
                new_stack_df.to_csv('stack_HydrateRidge_upper_2.02_MP_2min.csv', index=False)
                ...
                y_values=np.column_stack(stackvalues[timeinds])
        ...
        utcstart_chunk=utcstart_chunk+CHUNK_LENGTH/2
        utcend_chunk=utcend_chunk+CHUNK_LENGTH/2
        ...
        try:
            pickle.dump([datetimearray,ar,yax],open("peaks_HydrateRidge_upper_2.02_MP_2min_stack.p","wb"))
        except:
            continue     
