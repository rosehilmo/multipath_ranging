...
CHUNK_FILE = "MOMAR_2008_MP_halfwidth_5min.csv" #"AlaskaFins_LA21_chunk.csv" #Name of saved call file
...
DET_PATH="MOMAR_2008_MP_halfwidth_5min.csv" #"AlaskaFins_LA21.csv" #Final save file
...
st_raw=read("/Users/wader/Desktop/whaletracks/whaletracks/detection/2008-01-09-0700-00-LSo7_-Z.mseed")
st_raw[0].stats.sampling_rate=st_raw[0].stats.sampling_rate
STARTTIME=st_raw[0].stats.starttime
ENDTIME=st_raw[1].stats.endtime
st_raw[0].data=np.concatenate([st_raw[0].data,st_raw[1].data])
...
def main(STARTTIME, ENDTIME,
    ...
    if os.path.isfile(CHUNK_FILE) and is_restart:
        ...
        auto_df_full = [pd.read_csv('auto_MOMAR_2008_MP_halfwidth_5min.csv')]
        stack_df_full = [pd.read_csv('stack_MOMAR_2008_MP_halfwidth_5min.csv')]
    else:
        analyzers = []
        auto_df_full = []
        stack_df_full = []
    ...
    while utcend > utcstart_chunk:
        ...
        for idx in range(1, num_sta+1-1):
            ...
            if MP_SPCT_FLAG and len(analyzer_j.df) > 1: #if MP_FLAG is True
                ...
                for det in range(0,len(analyzer_j.df)):
                    mp_event=analyzer_j.mp_picker(timesnew, valuesnew, utcstart_chunk, dur=.01, prominence=0,distance=.5,rel_height=.1)
                ...
                for det in range(0,len(analyzer_j.df)):
                    mp_event=analyzer_j.mp_picker(dettimes_chunk,autocorr_chunk, utcstart_chunk, dur=.1, prominence=0,distance=.5,rel_height=.1)
                ...
                new_auto_df.to_csv('auto_MOMAR_2008_MP_halfwidth_5min.csv', index=False)
                new_stack_df.to_csv('stack_MOMAR_2008_MP_halfwidth_5min.csv', index=False)
        ...
        utcstart_chunk=utcstart_chunk+CHUNK_LENGTH/5
        utcend_chunk=utcend_chunk+CHUNK_LENGTH/5
        ...
        try:
            pickle.dump([datetimearray,ar,yax],open("peaks_MOMAR_2008_MP_halfwidth_5min_stack.p","wb"))
        except:
            continue     
