...
if BELLHOP == False:
    ...
    [distance,t0,t1,t2,t3,t0_1,t1_1,t1_2]=ranging.basic_ranging(depth,ss,sed_speed,t,plotflag=True)
    mp_1_timing=np.subtract(t1_1,t0)
    #p_1_1_timing=np.subtract(t1_1,t0)
    mp_2_timing=np.subtract(t2,t1)

if BELLHOP == True:
    df_bellhop=pd.read_csv('Marianas_bellhop_arrivals_sedlayer.csv')
...
#peaks=pickle.load(open("peaks_Marianas_B19_edit_stack.p","rb"))
...
df_calls=pd.read_csv('Marianas_B19.csv')
df_calls['peak_time']=pd.to_datetime(df_calls['peak_time'])

"""
df1=pd.read_csv('stack_Marianas_B19.csv') 
...
t1=0
df1=pd.read_csv('auto_Marianas_B19.csv')  
...
for row in range(len(df1)):
    ...
    if len(unique_calls) > 0:
        ...
        if best_distance < 110 and REFLECTIVITY == True:
            ...
            autopeaks_mp1_save += [best_distance]
        
        autotimes_save += [date]
        auto_max += [np.log10(df['peaks'])]
        auto_snr += [(df['snr'])]
        auto_amp += [(df['db_amps'])]
...
dictrange = {'time': autotimes_save, 'range': autopeaks_mp1_save}
...
plt.scatter(autotimes_save,autopeaks_mp1_save,c=auto_amp,cmap="gist_ncar") 
plt.scatter(autotimes_save,autopeaks_mp2_save,c=auto_amp,marker='x',cmap="gist_ncar") 
#plt.scatter(autotimes_save[j],autopeaks_mp2_save[j],c="magenta",zorder=1)  
plt.grid(axis='both')
plt.title('Attempted ranges for mp1 and mp1 timings, Colors are log(amplitudes) of strongest arrival')
...
saveranges.to_csv('Marianas_auto_test.csv', index=False)
