...
STARTTIME = ("2012-02-22T00:00:00.000")  # oops
ENDTIME = ("2013-02-05T00:00:00.000")

#STARTTIME = ("2013-01-17T00:00:00.000")  # oops
#ENDTIME = ("2013-01-19T00:00:00.000")
...
def main(STARTTIME, ENDTIME,
    ...
    if os.path.isfile(CHUNK_FILE) and is_restart:
        ...
        auto_df_full = [pd.read_csv('auto_Marianas_B19_v2.csv')]
    ...
    while utcend > utcstart_chunk:
        ...
        for idx in range(1, num_sta+1):
            ...
            if FINFLAG:
                ...
                snr_freqwidth = 3
                # Event metrics
                prominence = 200  # 4 #.2 #.5 #min threshold   .1 for 0.3 second window
 ...
 # Add freq info for blue whales
            ...
            #fs = round(tr_filt.stats.sampling_rate)
            ...
            for mp_iter in range(numchunks):
                ...
                if MP_SPCT_FLAG and len(min_df_sub) >= 1:  # if MP_FLAG is True
                    ...
                    mp_event = analyzer_j.mp_picker(
                            dettimes_chunk, autocorr_chunk, start_utc, dur=.1, prominence=0, distance=3, rel_height=.5)
        ...
        new_auto_df.to_csv('auto_Marianas_B19_v2.csv', index=False)
