...
CHUNK_FILE = "PuertoRico_12.10_MP_halfwidth_20min.csv" #"AlaskaFins_LA21_chunk.csv" #Name of saved call file
...
PLOTFLAG_STACK = True
CLIENT_CODE = 'IRIS'
network="ZZ" #Network name "OO" for OOI, "7D" for Cascadia, "XF" for PuertoRico, "XO" for alaska, "YO" for ENAM, "9A" for Hawaii, "ZZ" for puerto rico
station= "XABV" #"LD41" # "B19" for PuertoRico station #Specific station, or '*' for all available stations "X06" for ENAM, "LOSW" for Hawaii, "XABV" for puerto rico
...
DET_PATH="PuertoRico_12.10_MP_halfwidth_20min.csv" #"AlaskaFins_LA21.csv" #Final save file
...
#STARTTIME = ("2013-01-18T00:30:00.000") #PuertoRico amp test
#ENDTIME = ("2013-01-18T09:00:00.000")

#STARTTIME = ("2015-12-27T18:34:00.000") #PuertoRico amp test #2
#ENDTIME = ("2015-12-27T22:00:00.000")
...
def main(STARTTIME, ENDTIME,
    ...
    if os.path.isfile(CHUNK_FILE) and is_restart:
        ...
        auto_df_full = [pd.read_csv('auto_PuertoRico_12.10_MP_halfwidth_20min.csv')]
        stack_df_full = [pd.read_csv('stack_PuertoRico_12.10_MP_halfwidth_20min.csv')]
    else:
        analyzers = []
        auto_df_full = []
        stack_df_full = []
    ...
    while utcend > utcstart_chunk:
        ...
        for idx in range(1, num_sta+1):
            ...
            if MP_SPCT_FLAG and len(analyzer_j.df) > 1: #if MP_FLAG is True
                ...
                for det in range(0,len(analyzer_j.df)):
                    mp_event=analyzer_j.mp_picker(timesnew, valuesnew, utcstart_chunk, dur=.01, prominence=0,distance=4.5,rel_height=.1)
                ...
                for det in range(0,len(analyzer_j.df)):
                    mp_event=analyzer_j.mp_picker(dettimes_chunk,autocorr_chunk, utcstart_chunk, dur=.1, prominence=0,distance=4.5,rel_height=.1)
                ...
                new_auto_df.to_csv('auto_PuertoRico_12.10_MP_halfwidth_20min.csv', index=False)
                new_stack_df.to_csv('stack_PuertoRico_12.10_MP_halfwidth_20min.csv', index=False)
        ...
        try:
            pickle.dump([datetimearray,ar,yax],open("peaks_PuertoRico_12.10_MP_halfwidth_20min_stack.p","wb"))
        except:
            continue     
