...
CHUNK_FILE = "AK_WD50_12.14_MP_halfwidth_20min.csv" #"AlaskaFins_LA21_chunk.csv" #Name of saved call file
...
PLOTFLAG_STACK = True
CLIENT_CODE = 'IRIS'
network="XO" #Network name "OO" for OOI, "7D" for Cascadia, "XF" for marianas, "XO" for alaska, "YO" for ENAM, "9A" for Hawaii, "ZZ" for puerto rico
station= "WD50" #"WD62" # "B19" for marianas station #Specific station, or '*' for all available stations "X06" for ENAM, "LOSW" for Hawaii, "XABV" for puerto rico
...
DET_PATH="AK_WD50_12.14_MP_halfwidth_20min.csv" #"AlaskaFins_LA21.csv" #Final save file
...
#STARTTIME = ("2013-01-18T00:30:00.000") #Marianas amp test
...
#STARTTIME = ("2019-01-05T23:07:00.000") #for Alaska fins #1
#ENDTIME = ("2019-01-06T08:00:00.000")
...
STARTTIME = ("2018-12-14T05:00:00.000") #for Alaska fins WD62 test
ENDTIME = ("2018-12-15T05:00:00.000")

#STARTTIME = ("2019-02-16T12:00:00.000") #for Alaska fins WD62 test
#ENDTIME = ("2019-02-16T23:59:00.000")

#STARTTIME = ("2019-04-25T07:10:00.000") #for marianas fins
...
#STARTTIME = ("2013-01-14T22:25:00.000") # for fin max call testing marianas #1
#ENDTIME = ("2013-01-15T12:00:00.000")

#STARTTIME = ("2012-03-12T19:30:00.000") # for fin max call testing marianas #2
#ENDTIME = ("2012-03-12T21:59:00.000")

#STARTTIME = ("2013-01-21T02:00:00.000") # for fin max call testing marianas #3
#ENDTIME = ("2013-01-21T06:00:00.000")

#STARTTIME = ("2012-03-30T21:00:00.000") # for fin max call testing marianas #4
#ENDTIME = ("2012-03-30T23:00:00.000")

#STARTTIME = ("2013-01-19T22:08:00.000000Z") # for fin automation Marianas
...
def main(STARTTIME, ENDTIME,
    ...
    if os.path.isfile(CHUNK_FILE) and is_restart:
        ...
        auto_df_full = [pd.read_csv('auto_AK_WD50_12.14_MP_halfwidth_20min.csv')]
        stack_df_full = [pd.read_csv('stack_AK_WD50_12.14_MP_halfwidth_20min.csv')]
    else:
        analyzers = []
        auto_df_full = []
        stack_df_full = []
    ...
    while utcend > utcstart_chunk:
        ...
        for idx in range(1, num_sta+1):
            ...
            if FINFLAG:
                ...
                snr_freqwidth=5
                #Event metrics
                prominence= 2000 #4 #.2 #.5 #min threshold   .1 for 0.3 second window
            ...
            if MP_SPCT_FLAG and len(analyzer_j.df) > 1: #if MP_FLAG is True
                ...
                for det in range(0,len(analyzer_j.df)):
                    mp_event=analyzer_j.mp_picker(timesnew, valuesnew, utcstart_chunk, dur=.01, prominence=0,distance=2.5,rel_height=.1)
                ...
                for det in range(0,len(analyzer_j.df)):
                    mp_event=analyzer_j.mp_picker(dettimes_chunk,autocorr_chunk, utcstart_chunk, dur=.1, prominence=0,distance=2.5,rel_height=.1)
                ...
                new_auto_df.to_csv('auto_AK_WD50_12.14_MP_halfwidth_20min.csv', index=False)
                new_stack_df.to_csv('stack_AK_WD50_12.14_MP_halfwidth_20min.csv', index=False)
            ...
            if PLOTFLAG_STACK and len(analyzer_j.df) > 1:  #plots stacked spectrogram
                ...
                fig.tight_layout()
                fig.savefig('Figures/Alaska/AK_test' + str(utcstart_chunk) + '.png', dpi=100)
                #plt.show()
                plt.close()
            #Calculate SNR info
        ...
        try:
            pickle.dump([datetimearray,ar,yax],open("peaks_AK_WD50_12.14_MP_halfwidth_20min_stack.p","wb"))
        except:
            continue     
