...
CHUNK_FILE = "ENAM_2.28_20min_halfwidth_10s_newkernel.csv" #"AlaskaFins_LA21_chunk.csv" #Name of saved call file
...
PLOTFLAG_STACK = True
CLIENT_CODE = 'IRIS'
network="YO" #Network name "OO" for OOI, "7D" for Cascadia, "XF" for PuertoRico, "XO" for alaska, "YO" for ENAM, "9A" for Hawaii, "ZZ" for puerto rico
station= "X08" #"LD41" # "B19" for PuertoRico station #Specific station, or '*' for all available stations "X06" for ENAM, "LOSW" for Hawaii, "XABV" for puerto rico
...
DET_PATH="ENAM_2.28_20min_halfwidth_10s_newkernel.csv" #"AlaskaFins_LA21.csv" #Final save file
...
#TARTTIME = ("2015-12-10T22:00:00.000") # for fin testing puerto rico #1
...
#STARTTIME = ("2013-01-18T00:30:00.000") #Mar amp test
#ENDTIME = ("2013-01-18T09:00:00.000")

#STARTTIME = ("2015-12-27T18:34:00.000") #Mar amp test #2
#ENDTIME = ("2015-12-27T22:00:00.000")
...
#STARTTIME = ("2014-09-10T11:00:00.000") #for ENAM fins MP X08 1
#ENDTIME = ("2014-09-11T17:00:00.000")

STARTTIME = ("2015-02-28T20:30:00.000") #for ENAM fins MP X08 later,
ENDTIME = ("2015-03-01T03:30:00.000")

#STARTTIME = ("2014-12-27T02:30:00.000") #for ENAM fins MP X08 BEST but two whales
#ENDTIME = ("2014-12-27T11:30:00.000")

#STARTTIME = ("2014-12-27T06:00:00.000") #for ENAM fins MP X08 BEST but two whales for subsurface
#ENDTIME = ("2014-12-27T08:00:00.000")
...
#STARTTIME = ("2019-04-25T07:10:00.000") #for PuertoRico fins
...
#STARTTIME = ("2013-01-14T22:25:00.000") # for fin max call testing PuertoRico #1
#ENDTIME = ("2013-01-15T12:00:00.000")

#STARTTIME = ("2012-03-12T19:30:00.000") # for fin max call testing PuertoRico #2
#ENDTIME = ("2012-03-12T21:59:00.000")

#STARTTIME = ("2013-01-21T02:00:00.000") # for fin max call testing PuertoRico #3
#ENDTIME = ("2013-01-21T06:00:00.000")

#STARTTIME = ("2012-03-30T21:00:00.000") # for fin max call testing PuertoRico #4
#ENDTIME = ("2012-03-30T23:00:00.000")

#STARTTIME = ("2013-01-19T22:08:00.000000Z") # for fin automation PuertoRico
...
def main(STARTTIME, ENDTIME,
    ...
    if os.path.isfile(CHUNK_FILE) and is_restart:
        ...
        auto_df_full = [pd.read_csv('auto_ENAM_2.28_20min_halfwidth_10s_newkernel.csv')]
        stack_df_full = [pd.read_csv('stack_ENAM_2.28_20min_halfwidth_10s_newkernel.csv')]
    else:
        analyzers = []
        auto_df_full = []
        stack_df_full = []
    ...
    while utcend > utcstart_chunk:
        ...
        for idx in range(1, num_sta+1):
            ...
            if FINFLAG:
                ...
                distance=8#minimum distance between detections
                rel_height=.8
            ...
            if MP_SPCT_FLAG and len(analyzer_j.df) > 1: #if MP_FLAG is True
                ...
                for det in range(0,len(analyzer_j.df)):
                    mp_event=analyzer_j.mp_picker(timesnew, valuesnew, utcstart_chunk, dur=.01, prominence=-1,distance=4,rel_height=.5)
                ...
                for det in range(0,len(analyzer_j.df)):
                    mp_event=analyzer_j.mp_picker(dettimes_chunk,autocorr_chunk, utcstart_chunk, dur=.01, prominence=-1,distance=4,rel_height=.5)
                ...
                new_auto_df.to_csv('auto_ENAM_2.28_20min_halfwidth_10s_newkernel.csv', index=False)
                new_stack_df.to_csv('stack_ENAM_2.28_20min_halfwidth_10s_newkernel.csv', index=False)
            ...
            if PLOTFLAG_STACK and len(analyzer_j.df) > 1:  #plots stacked spectrogram
                ...
                fig.tight_layout()
                fig.savefig('Figures/Hawaii/window_0_8s/stackspect' + str(utcstart_chunk) + '.png', dpi=100)
                #plt.show()
                plt.close()
            #Calculate SNR info
        ...
        try:
            pickle.dump([datetimearray,ar,yax],open("peaks_ENAM_2.28_20min_halfwidth_10s_stack.p","wb"))
        except:
            continue     
    ...
    #pickle.dump([datetimearray,ar,yax],open("peaks_PuertoRico_stack.p","wb"))
    #import pdb; pdb.set_trace()
